...
public final class ConnectionManager {
  ...
  private final Map<Address, CompletableFuture<Connection>> futures = new HashMap<>();
  ...
  private CompletableFuture<Connection> createConnection(Address address) {
    return futures.computeIfAbsent(address, a -> {
      ...
      future.whenComplete((connection, error) -> futures.remove(address));
      return future;
    });
  }
  ...
  public CompletableFuture<Void> close() {
    ...
    for (CompletableFuture<Connection> future : this.futures.values()) {
      future.cancel(false);
    }
